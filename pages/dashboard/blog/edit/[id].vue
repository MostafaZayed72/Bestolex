  ...
  <NuxtLayout name="dashboard">
    <div class="max-w-4xl mx-auto space-y-8">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-2xl font-black text-gray-900 dark:text-white">تعديل المقال</h2>
          <p class="text-gray-500 dark:text-gray-400 text-sm">تعديل بيانات ونصوص المقال باللغتين العربية والإنجليزية</p>
        </div>
      ...
      <!-- Loading State -->
      <div v-if="isLoading" class="bg-white dark:bg-gray-800 p-12 rounded-3xl text-center text-gray-400">
        <div class="animate-spin text-3xl mb-2">⏳</div>
        <div>جاري تحميل بيانات المقال...</div>
      </div>

      <!-- Main Form -->
      <form v-else @submit.prevent="updateArticle" class="space-y-6 bg-white dark:bg-gray-800 p-6 sm:p-10 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm">
        ...
        <div v-show="activeLang === 'ar'" class="space-y-6">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">القسم (نقل المقال إلى قسم آخر)*</label>
              <select 
...
onMounted(async () => {
  try {
    ...
    if (cats) categoryList.value = cats

    if (data) {
      form.value = {
        title_ar: data.title_ar || data.title?.ar || '',
        title_en: data.title_en || data.title?.en || '',
        category_ar: data.category_ar || data.category?.ar || '',
        category_en: data.category_en || data.category?.en || '',
        read_time_ar: data.read_time_ar || data.readTime?.ar || '5 دقائق قراءة',
        read_time_en: data.read_time_en || data.readTime?.en || '5 min read',
        excerpt_ar: data.excerpt_ar || data.desc?.ar || '',
