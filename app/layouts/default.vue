<template>
  <div :class="{ 'dark': isDark }" class="min-h-screen bg-white dark:bg-secondary text-gray-900 dark:text-white transition-colors duration-300 flex flex-col">
    ...
    <div class="fixed bottom-4 flex flex-col gap-4 z-50" :class="locale === 'ar' ? 'left-4' : 'right-4'">
      ...
      <a href="https://wa.me/97451505014" target="_blank" rel="noopener noreferrer" class="w-12 h-12 bg-[#25D366] hover:bg-[#1eb954] text-white rounded-full shadow-lg flex items-center justify-center transition transform hover:scale-110 hover:shadow-green-500/50" data-aos="fade-up" aria-label="WhatsApp">
